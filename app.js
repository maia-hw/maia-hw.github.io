...
let NUM_LETTERS = 5;
let MIN_INVERSLES = 2; //the inversle will not hide a word with less than 2 solutions - not fun enough

//
// all valid solutions to the wordle (common words)
//
...
// each letter position either is or isn't something that was already guessed for this position
class Letter {
    constructor() {
        this.is = null;
        this.isNot = new Set();
...
// greens, yellows, and blacks for the entire word
...
// each guess is one node in the doubly-linked list Wordle, and has a knowledge state associated with it
class Guess {
    constructor(word) {
        this.guess = word;
...
// Each guess in the solution to the wordle is linked to the next and the previous, for easy iteration
class Wordle {
    constructor(word) {
        this.solution = word;
        this.found = false;
    ...
    makeGuess(word) {
        ...
        if (this.length === 0) {
            ...
            this.last.next = newGuess;
            newGuess.prev = this.last;
            this.last = newGuess;
...
// validate whether a guess is a word or not
function isValidGuess(guess) {
    if (valid_guesses.indexOf(guess.toLowerCase()) == -1 && valid_solutions.indexOf(guess.toLowerCase()) == -1) {
        return false;
    }
    return true;
}
...
function consolidateWordle(wordle) {
    ...
    while (currGuess.next != null && currGuess.next.next != null) {
        if (coloursMatch(currGuess.next.colours, currGuess.colours) && currGuess.colours.indexOf("wrong-spot") == -1) {
            ...
            // don't move on untill we're out of repeats
            continue;
        }
...
function toggleDarkLight() {
    ...
    const switcher = document.getElementById("darkmode");
    ...
    if (className == "light-theme") {
        ...
        document.getElementById("help").style.backgroundImage = "url('icons/help-light.jpg')";
    } else {
        switcher.style.backgroundImage = "url('icons/sun.jpg')";
        switcher.textContent = "Light";
        document.getElementById("stats").style.backgroundImage = "url('icons/stats-dark.jpg')";
        document.getElementById("help").style.backgroundImage = "url('icons/help-dark.jpg')";
    }
...
/* ***********************************************
 *                                               *
 *     Main function (also an event listener)    *
 *                                               *  
 * ***********************************************/
